---
sidebar_position: 1
sidebar_label: "What is RedStone?"
sidebar_class_name: sidebar-redstone-section
---

# Introduction

The documentation is organized around the RedStone Stack, a three-stage architecture covering data delivery, capital efficiency, and risk intelligence.

---

### Stage 1: Market Data

The base layer. Every protocol needs reliable price data before anything else can work.

RedStone delivers on-chain price feeds, data streaming, and risk infrastructure to 200+ protocols across 110+ chains, with more than billions in collateral secured and no mispricing or downtime events on record.

Stage 1 includes push feeds for lending markets and vaults, pull feeds for gas-efficient integrations, RedStone Bolt for high-throughput chains where block times fall below the cadence of standard oracles, RedStone Live for off-chain streaming to perpetual exchanges and trading systems, and Proof of Reserve for tokenized assets that require on-chain verification of their backing.

### Stage 2: Capital Efficiency

Addresses what happens after price data arrives.

RedStone Settle provides instant liquidity for RWA collateral, bridging the gap between DeFi's instant settlement requirement and the redemption windows of tokenized real-world assets. RedStone Atom captures Oracle Extractable Value by running sealed-bid auctions before liquidation-triggering price updates go on-chain, returning value to protocols rather than MEV bots.

### Stage 3: Risk Intelligence

Adds credit assessment to the stack.

Credora provides continuous, on-chain risk ratings for DeFi tokens, loan pairs, and vaults, built on methodologies that translate to traditional credit equivalents. This gives curators and lenders an independent view of what they are actually lending against, beyond what a price feed alone can tell them.

---

Most protocols use more than one product in the stack. Start with the stage most relevant to your use case and work outward from there.

[Contact RedStone](https://redstone.finance/contact) for integration support.
