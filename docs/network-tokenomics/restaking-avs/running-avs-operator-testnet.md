...
```bash
docker run --platform linux/arm64 -it public.ecr.aws/z6z4q0a5/prod-avs-othentic-client:b52e8208 \
  operator register \
  --l1-chain holesky
```
...
required: [Attester](/docs/network-tokenomics/restaking-avs/service-components#attester)
and [Validation API](/docs/network-tokenomics/restaking-avs/service-components#validation-api).
